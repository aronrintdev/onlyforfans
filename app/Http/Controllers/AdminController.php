...
//            }
    ...
    public function removeCategory(Request $request)
    {
        ...
        if($category == null)
        {
            return response()->json(['status' => '201', 'category' => false, 'message' => trans('messages.noty_category_not_found')]);
        }

        $categories = Category::where('id','!=', $request->category_id)->where('parent_id',$request->category_id)->get();
        if(!$categories->isEmpty())
        {
            return response()->json(['status' => '201', 'category' => false, 'message' => trans('messages.noty_category_delete_error_subcategory')]);
        }
        
        $page = Page::where('category_id', $request->category_id)->get();
        if(!$page->isEmpty())
        {
            return response()->json(['status' => '201', 'category' => false, 'message' => trans('messages.noty_category_delete_error')]);
        }
        if ($category->delete()) {
            Flash::success(trans('messages.category_deleted_success'));

            return response()->json(['status' => '200', 'category' => true, 'message' => trans('messages.noty_category_deleted')]);
        }
    }
...
//        }
    ...
    protected function validateUser(array $data, $timeline_id, $user_id)
    {
        ...
        return Validator::make($data, [
            'username' => 'required|max:16|min:5|alpha_num|no_admin|unique:timelines,username,'.$timeline_id,
            'name'     => 'required',
    ...
    public function postUpdateDatabase(Request $request)
    {
        ...
        $output = Artisan::output();
        $settings = [   'group_event_privacy'         => 'only_admins',
                        'footer_languages'            => 'on',
