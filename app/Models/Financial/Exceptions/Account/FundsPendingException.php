 ...
 */
class FundsPendingException extends RuntimeException
{
    protected $account;
    ...
    public function make($account, $transactionAmount, $balanceResult)
    {
        ...
        $this->message = "Insufficient Funds to make transaction due to pending funds! Account [{$account->getKey()}] with balance of [{$account->balance->getAmount()}] and pending of [{$account->pending->getAmount()}] cannot make transaction of [{$transactionAmount}] as it would result in a balance of [{$balanceResult}]";
    }
}
